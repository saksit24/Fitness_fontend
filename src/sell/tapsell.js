...
class Home extends Component {
  ...
  slip_product = async () => {
    let object = {
      ...
      date_product: this.state.date_product,
      type_product: this.state.type_product,
      quantity_product: this.state.quantity_product,
      price_product: this.state.price_product,
      sum_price_product: this.state.sum_price_product,
      get_price_product: this.state.get_price_product,
      change_price_product: this.state.change_price_product
    };
    console.log("product", object);


    try {
      await post(object, "product/slip_product", null).then(result => {
...

            {
              this.state.show_addtocart.map((element, index) => {
                return (
                  ...
                  <tr style={{ border: "1px solid #000" }}> {console.log("show_addtocart", element)}
                    <td style={{ border: "1px solid #000", textAlign: "center" }}>{index + 1}</td>
                    {/* <td style={{ border: "1px solid #000" }}>{moment(element.date_product).format('DD/MM/YYYY HH:mm')}</td> */}
